...
from project.esg_framework_sustainalytics.heuristics_sustainalytics import (
    ALL_DOMAINS,
    DOMAIN_HEURISTICS,
    DOMAIN_KEYWORDS,
    DOMAIN_SCORE_PRIORS,
...
def score_label(score: float) -> str:
    """Sustainalytics uses 0-100 MEI scale."""
    if score < 40:
        return "low"
    if score < 70:
        return "medium"
...
def _normalize_score(raw_hits: int, token_count: int) -> float:
    """Scaled for 0-100 range (was 120 for 0-20 scale, now 600 for 0-100)."""
    density = raw_hits / max(token_count, 1)
    calibrated = min(MAX_SCORE, max(0.0, density * 600.0))
...
def _build_domain_prompt(domain: str, chunks: list[Chunk]) -> str:
    ...
    # Sustainalytics-aligned scoring rubric (0-100 scale)
    rubric = """
    Scoring Rubric (Sustainalytics Management Indicator, 0-100 scale):
    - 0: No evidence of management (e.g., no policy, no disclosure, or no relevant data).
    - 25: Weak policy or minimal evidence (e.g., generic statements, no implementation, or partial disclosure).
    - 50: Adequate policy (e.g., basic commitments, partial implementation, or some measurable targets).
    - 75: Strong policy (e.g., clear targets, governance ownership, monitoring, and outcome-focused indicators).
    - 100: Very strong policy (e.g., quantified targets, third-party verification, full integration, and proven track record).
    """

    # Sustainalytics-aligned domain heuristics for the 5 MEI thematic areas
    domain_heuristics = {
        "product_production": {
            "checklist": [
                "Identify Material ESG Issues (MEIs) relevant to Product & Production (e.g., Carbon Emissions, Water Use, Biodiversity, Waste Management, Resource Efficiency).",
                "Assess Exposure: Evaluate subindustry exposure scores (2-10) and company-specific Beta Indicators for Product & Production domain.",
                "Assess Management: Check for policies, targets, and performance data (e.g., CO2 emissions, energy efficiency, certifications, waste reduction).",
                "Evaluate Event Indicators: Look for controversies or incidents (scale 0-5) related to production processes.",
                "Calculate Unmanaged Risk: Use the formula: (Issue Exposure * MRF) - Managed Risk.",
                "Note: MRF (Manageable Risk Factor) ranges from 30% to 100% and is predefined at the subindustry level.",
            ],
            "prior": 50  # Neutral prior (midpoint of 0-100 scale)
        },
        "financials": {
            "checklist": [
                "Identify Material ESG Issues relevant to Financials (e.g., Solvency risks, capital allocation, financial stability).",
                "Assess Exposure: Evaluate financial vulnerability to ESG risks using Beta Indicators.",
                "Assess Management: Review financial risk management, ESG integration in financial planning, stress testing.",
                "Evaluate Financial Resilience: Check for adequate capital buffers, liquidity management, and ESG-linked financing.",
                "Calculate Unmanaged Risk: Use the formula: (Issue Exposure * MRF) - Managed Risk.",
            ],
            "prior": 50
        },
        "events": {
            "checklist": [
                "Identify Controversial or High-Impact Events (e.g., environmental incidents, corruption scandals, human rights violations, major controversies).",
                "Assess Event Severity: Use Category 4 (high impact) or Category 5 (severe impact) for material events.",
                "Evaluate Event Indicators: Check scale (0-5) and materiality of each event.",
                "Assess Company Response: Review remediation actions, transparency, and preventative measures.",
                "Calculate Exposure Adjustment: Increase exposure score by 6 (Category 4) or 8 (Category 5) points.",
            ],
            "prior": 40
        },
        "geographic": {
            "checklist": [
                "Evaluate Geographic Exposure risks (e.g., water risk in regions with scarcity, regional corruption levels, political stability).",
                "Assess Regional Regulatory Environments: Review local ESG regulations and compliance requirements.",
                "Evaluate Resource Nationalism: Check for risks related to local content requirements, export restrictions.",
                "Assess Cross-Border Risks: Evaluate geopolitical risks, trade restrictions, and jurisdiction-specific issues.",
                "Use Geographic Beta Indicators to refine exposure assessments.",
            ],
            "prior": 45
        },
        "governance": {
            "checklist": [
                "Assess Corporate Governance: Evaluate board structure, independence, transparency, accountability, and alignment of owner-manager interests.",
                "Assess Stakeholder Governance: Review stakeholder engagement, risk mitigation, and reputational management.",
                "Check for Event Indicators: Look for governance controversies (e.g., fraud, corruption, regulatory fines, board disputes).",
                "Evaluate Management Indicators: Assess policies (e.g., anti-corruption, board diversity, executive compensation, whistleblower protection) and their implementation.",
                "Note: Governance is always material and cannot be disabled for any company. Default exposure: 70 for Corporate Governance (public), 20 for Stakeholder Governance (public).",
            ],
            "prior": 60
        }
    }

    # Get domain-specific heuristic or default to generic
    heuristic = domain_heuristics.get(domain, {
        "checklist": [f"Review evidence for relevance to {domain} ESG risks."],
        "prior": 50
    })
    heuristic_checklist = "\n".join(heuristic["checklist"])
    ...
    return (
        ...
        f"Apply this Sustainalytics-aligned domain heuristic checklist:\n{heuristic_checklist}\n\n"
        "Scoring Rubric (Sustainalytics Management Indicator, 0-100 scale):\n"
        "- 0: No evidence of management (e.g., no policy, no disclosure, or no relevant data).\n"
        "- 25: Weak policy or minimal evidence (e.g., generic statements, no implementation, or partial disclosure).\n"
        "- 50: Adequate policy (e.g., basic commitments, partial implementation, or some measurable targets).\n"
        "- 75: Strong policy (e.g., clear targets, governance ownership, monitoring, and outcome-focused indicators).\n"
        "- 100: Very strong policy (e.g., quantified targets, third-party verification, full integration, and proven track record).\n\n"
        "Scoring Discipline (Sustainalytics Management Indicator methodology):\n"
        f"1) Start near a neutral prior of {heuristic['prior']} and adjust based on explicit evidence.\n"
...
def _calibrate_domain_score(
    ...
    prior = DOMAIN_SCORE_PRIORS.get(domain, 50.0)
    ...
    # Scaled for 0-100: prior + 20 + (55 * quality) maintains same relative proportions
    evidence_cap = min(MAX_SCORE, prior + 20.0 + (55.0 * quality))
    ...
    rationale = (
        ...
        f"Detected calibration anchor from heuristic and prior ({prior:.2f}) with rubric gating. "
...
def _build_critique_prompt(
    ...
    return (
        ...
        f"Domain: {domain}\n"
        f"Candidate's current Management Indicator score: {candidate_score}/100\n"
...
def critique_and_adjust(domain: str, candidate: DomainScore, critique_chunks: list[Chunk]) -> tuple[DomainScore, dict[str, float | bool]]:
    ...
    # Scaled threshold: 1.0 -> 5.0 for 0-100 scale
    needs_adjustment = gap > 5.0 or (has_feedback and adjustment_direction != "no_change")
    ...
    # Scaled threshold: 5.0 -> 25.0 for 0-100 scale
    if gap > 25.0:
